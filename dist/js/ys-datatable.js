...
(function(factory) {
  ...
  const YSDataTable = function(options = {}) {
    ...
    if (this.prop("rows") !== null && this.prop("rows") !== undefined) {
      ...
      if (
        options.removeButtons !== undefined &&
        options.removeButtons !== null
      ) {
        options.removeButtons.target.map(
          target => delete options.buttons[Table.BMAP[target]]
        );
      }
      // setting column name case
      const columnCase = this.prop("tHead").attributes.case
        ? this.prop("tHead").attributes.case.value
        : "lower";
      Table.prototype.setColumnNameCase(columnCase);
      // setting properties of columns from tags
      options.columns = Table.prototype.getColumnsFromTable(
        this.prop("rows")[0]
      );
  ...
  const Table = function({ el, options }) {
     ...
     */
    this.exportable = this.getExportable();

    /**
     * Uri to server side export
     * @type {String}
     */
    this.exportUri = "ys-datatable/export";

    /**
     * Case used for column names (values : upper/lower)
     */
    this.columnNameCase = el.prop("tHead").attributes.case
      ? el.prop("tHead").attributes.case.value
      : "lower";
    ...
    this.defaults = {
      ...
      columnDefs: null,
      dom:
        '<"row"<"col-sm-2 "l><"col-sm-8"B><"col-sm-2"f>r>t<"row"<"col-sm-12"i>><"row"<"col-sm-12 center-block"p>>',
      ajax: {},
      processing: false,
      serverSide: true
    };
    ...
    /**
     * Checks whether options object contain download option
     * and if present than enable server-side export of data
     * @return {void}
     */
    if (options.Export !== undefined && options.Export !== null) {
      if (options.Export.fun && options.Export.model && options.Export.uri) {
        this.exportUri =
          options.Export.uri +
          "?model=" +
          options.Export.model +
          "&fun=" +
          options.Export.fun;
      } else if (options.Export.uri) {
        this.exportUri = options.Export.uri;
      } else if (options.Export.fun) {
        this.exportUri +=
          "?model=" + options.Export.model + "&fun=" + options.Export.fun;
      } else if (options.Export.model) {
        this.exportUri += "?model=" + options.Export.model;
      }

      this.addServeSiderExportButton();
    }
  ...
  Table.prototype = {
    ...
    getColumnsFromTable(tHead) {
      return Array.from(tHead.cells).map((th, index) => {
        if (th.attributes.export) {
          ...
          if (
            parseInt(th.attributes.export.value, 10) === 1 ||
            th.attributes.export.value == "true"
          ) {
        ...
        const columns = {
          ...
          sortable: th.attributes.sort
            ? !!parseInt(th.attributes.sort.value, 10)
            : true,

          visible: th.attributes.visible
            ? !!parseInt(th.attributes.visible.value, 10)
            : true,

          class: th.attributes.dtClass ? th.attributes.dtClass.value : "",

          defaultContent: th.attributes.defaultContent
            ? th.attributes.defaultContent.value
            : null,
        ...
        $.extend(columns, {
          data:
            th.dataset.name !== undefined && th.dataset.name !== null
              ? this.slugify(th.dataset.name)
              : this.slugify(th.innerHTML)
        });
    ...
    slugify(str, delimeter = "_") {
      str = str.trim().replace(/ +/g, delimeter);
    ...
    /**
     * Add server side export button to default buttons
     * @return {void}
     */
    addServeSiderExportButton() {
      let uri = this.exportUri;
      this.defaults.buttons = this.defaults.buttons.concat([
        {
          text:
            '<span><i class="fa  fa-file fa-lg"></i> Export All To Csv </span>',
